...
3. dmcp：使用xbrower（xmanager中的组件）。CentOS下安装并配置gdm，即可登录。
   参考：https://www.zybuluo.com/wuzhimang/note/392271
   yum install lightdm
...
1. Chrome浏览器在linux是否可以使用（无法下载）
   碰到的问题：
   a. Chrome安装过程中，是在/etc/yum.repo.d/下生成了一个软件源配置文件，直接访问google网站不通。需要配置系统代理。
   b. 配置系统代理后，又发生证书没有导入的问题，需要导入证书。
2. Linux中的中文输入法
